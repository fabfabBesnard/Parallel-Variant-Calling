#!/bin/bash
## script: 'GATK_fq-to-gVCF.v4_PSMN.sh'
## Fabrice Besnard, 2018 v4, 
...
#Usage:
#sh GATK_fq-to-gVCF.sh Config-file_mysample.txt
...
####################################
printf "${col} **Running GATK_fq-to-gVCF.v4_PSMN.sh** ${NC} \n"
...
printf "${col} Final file generated will be: ${NC} $OUTPUT_NAME.gVCF.vcf, where 'sample' is $RGSM \n"
...
################################
#Check the vcf
if [ -e $OUTPUT_NAME.gVCF.vcf ]; then    
	printf "${col} **END OF SCRIPT** Variants of $OUTPUT_NAME called and stored into $OUTPUT_NAME.gVCF.vcf ${NC} \n"
else     
	printf "\e[0;31m ##ERROR## Expected gvCFfile was not generated. Last step failed ${NC} \n"
	exit 1
fi
#Check the bam
